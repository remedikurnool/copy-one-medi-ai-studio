...
const CategoryPill = ({ icon, label, color, onClick }: any) => (
  <button onClick={onClick} className="flex flex-col items-center gap-2 min-w-[72px] group transition-transform active:scale-95">
    <div className={`size-[4.5rem] rounded-2xl flex items-center justify-center shadow-glass border border-white/50 backdrop-blur-sm transition-all group-hover:-translate-y-1 ${color}`}>
      <span className="material-symbols-outlined text-3xl drop-shadow-sm">{icon}</span>
    </div>
...
const SectionHeader = ({ title, onSeeAll }: { title: string, onSeeAll: () => void }) => (
  <div className="flex items-center justify-between mb-4 px-1">
    ...
    <button onClick={onSeeAll} className="text-xs font-bold text-primary hover:bg-primary/5 px-2 py-1 rounded-lg transition-colors flex items-center gap-0.5">
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col relative isolate min-h-screen pb-24">
      <BackgroundBlobs />
      <LocationModal isOpen={isLocationModalOpen} onClose={() => setIsLocationModalOpen(false)} />

      {/* Redesigned Rounded Header */}
      <header className="sticky top-0 z-40 bg-primary dark:bg-gray-900 shadow-xl rounded-b-[2rem] transition-all duration-300 pb-2">
      ...
      <section className="mt-6 px-4 animate-slide-up relative z-10">
      ...
      <section className="px-4 mb-6">
        <button 
          ...
          className="w-full bg-red-500 hover:bg-red-600 text-white rounded-2xl p-4 shadow-lg shadow-red-500/30 flex items-center justify-between active:scale-[0.98] transition-all group"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-bold text-lg leading-tight">Emergency SOS</h3>
              <p className="text-xs text-red-100 opacity-90">Call Ambulance in Kurnool</p>
            </div>
          </div>
          <div className="size-8 bg-white text-red-500 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">
            <span className="material-symbols-outlined">call</span>
          </div>
        </button>
      </section>

      {/* Prescription Promo Banner */}
      <section className="px-4 mb-8 animate-slide-up relative z-10" style={{animationDelay: '0.05s'}}>
        <PrescriptionPromo />
      </section>

      {/* Bento Grid Services */}
      <section className="px-4 mb-8 animate-slide-up relative z-10" style={{animationDelay: '0.1s'}}>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4 px-1">Top Services</h2>
        <div className="grid grid-cols-2 gap-4">
           ...
           <BentoCard 
             ...
             icon="science"
             bgClass="bg-gradient-to-br from-teal-50 to-emerald-50 dark:from-teal-900/20 dark:to-emerald-900/20 h-32"
           ...
           <BentoCard 
             ...
             icon="stethoscope"
             bgClass="bg-gradient-to-br from-purple-50 to-fuchsia-50 dark:from-purple-900/20 dark:to-fuchsia-900/20 h-32"
           ...
           <BentoCard 
             ...
             title="Book Scans & MRI"
             subtitle="Compare Prices from Verified Centers"
             icon="radiology"
             bgClass="bg-gradient-to-r from-slate-100 to-gray-100 dark:from-slate-800 dark:to-gray-800"
      ...
      <section className="px-4 mb-8 animate-slide-up relative z-10" style={{animationDelay: '0.2s'}}>
        <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 no-scrollbar pb-4">
           {/* Banner 1 */}
           <div className="snap-center shrink-0 w-[90%] sm:w-[60%] h-48 rounded-3xl overflow-hidden relative shadow-lg group cursor-pointer active:scale-[0.98] transition-transform" onClick={() => navigate('/lab-tests')}>
              <div className="absolute inset-0 bg-gradient-to-r from-blue-900 via-blue-800/80 to-transparent z-10 flex flex-col justify-center p-6 pl-8">
                 ...
                 <h3 className="text-white text-2xl font-extrabold leading-tight mb-2 drop-shadow-md">Full Body Check<br/><span className="text-blue-200">@ Just ₹999</span></h3>
              ...
              <div className="absolute right-0 top-0 h-full w-2/3 bg-cover bg-center" style={{backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuChcVgUocqxKyeU56-z1f-Wwte61zbuAgJy_oZML3U2kDcbkG0b5ZqbAU5HUqZZprD_jHs_gsCWzBTt5HUbTFu0OBFqDEuB0hAWvRZDmkmhXEaVs1chYNNpi1rS6CkCy2Rd_0H1ss0AdJjRnL-YJ3O-1HOoAErxtG28Ck0Vsx3DEqnYPsFQsohbYcRbARh0EUQ7SP6Dzc_BWcEG9JdUJOC_VUy5n87yiR_HTao37qp931dMDL9BJe3apC6IaQsfIv_gKnb-Sv_v0bw")'}}></div>
           </div>
           
           {/* Banner 2 */}
           <div className="snap-center shrink-0 w-[90%] sm:w-[60%] h-48 rounded-3xl overflow-hidden relative shadow-lg group cursor-pointer active:scale-[0.98] transition-transform" onClick={() => navigate('/medicines')}>
              <div className="absolute inset-0 bg-gradient-to-r from-teal-900 via-teal-800/80 to-transparent z-10 flex flex-col justify-center p-6 pl-8">
                 ...
                 <h3 className="text-white text-2xl font-extrabold leading-tight mb-2 drop-shadow-md">Medicines in<br/><span className="text-teal-200">2 Hours</span></h3>
              ...
              <div className="absolute right-0 top-0 h-full w-2/3 bg-cover bg-center" style={{backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuCX0N7APw5Sja-YQkfMLdSfrk3dTZtx3i44lcrNwkG3onjfl0cfXDzwrVuz8QWbp6esJWZ6nKjQeDWvvJCxqNrKCxMwYmG_vdHIDBu4kW8sbXZsj69bmHN2ZnA6gRsvyBD1E_eyo-l4f6E8BI1ZeRFL_KfEhFHCocpyBlaGyXAIv7bEM8pkt8I_fmTmv9sQ3XS4OOwoZSscczXPfkG7FaeFNDYJX963sZSASinr_g9xQv8jfEDPtzQu-bQA7hZ1QboauciDfU7ZHh8")'}}></div>
           </div>
        </div>
      </section>

      {/* Best Sellers Carousel */}
      <section className="mb-8 pl-4 animate-slide-up relative z-10" style={{animationDelay: '0.3s'}}>
         <SectionHeader title="Best Sellers" onSeeAll={() => navigate('/medicines')} />
         <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 snap-x snap-mandatory">
            {MEDICINES.map((med) => (
              <MedicineCard key={med.id} medicine={med} onClick={() => navigate(`/medicines/${med.id}`)} />
            ))}
         </div>
      </section>

      {/* Popular Lab Tests Carousel */}
      <section className="mb-8 pl-4 animate-slide-up relative z-10" style={{animationDelay: '0.4s'}}>
         <SectionHeader title="Popular Tests" onSeeAll={() => navigate('/lab-tests')} />
         <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 snap-x snap-mandatory">
            {LAB_TESTS.map((test) => (
              <LabTestCard key={test.id} test={test} onClick={() => navigate(`/lab-tests/${test.id}`)} />
            ))}
         </div>
      </section>

      {/* Most Booked Scans Carousel */}
      <section className="mb-8 pl-4 animate-slide-up relative z-10" style={{animationDelay: '0.5s'}}>
         <SectionHeader title="Most Booked Scans" onSeeAll={() => navigate('/scans')} />
         <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 snap-x snap-mandatory">
            {MEDICAL_SCANS.map((scan) => (
              <ScanCard key={scan.id} scan={scan} onClick={() => navigate('/scans/detail', { state: { scanId: scan.id } })} />
            ))}
         </div>
      </section>

      {/* Best Doctors in Kurnool Carousel */}
      <section className="mb-0 pl-4 animate-slide-up relative z-10" style={{animationDelay: '0.6s'}}>
         <SectionHeader title="Top Doctors in Kurnool" onSeeAll={() => navigate('/doctors')} />
         <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 snap-x snap-mandatory">
            {DOCTORS.map((doc) => (
              <DoctorCard key={doc.id} doctor={doc} onClick={() => navigate(`/doctors/${doc.id}`)} />
            ))}
         </div>
      </section>
    </div>
